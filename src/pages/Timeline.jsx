...
class Timeline extends React.Component {
  ...
  _sendRequest = () => {
    this.setState({ isLoading: true, error: "", result: null }, async () => {
      const data = await fetch(`${url}/revision-tools/timeline`, {
        method: "POST",
        body: JSON.stringify({
  ...
  render() {
    ...
    return (
      <>
        ...
        <Container>
          ...
          <Grid gutter={"sm"}>
            ...
            <Grid.Col sm={12} md={10}>
              <Title size="h1">{"Frise Chronologique"}</Title>
              <Text size={"md"}>
                {
        ...
        <Container>
          <Paper withBorder radius="md" className={classes.card} bg={"#242537"}>
            ...
            <TextInput
              size={"md"}
              label={"Sujet:"}
              placeholder={"Le sujet sur lequel faire la frise chronologique"}
              mb={"sm"}
            ...
            />
            <Textarea
              label={"Evenements à inclure (optionnel):"}
              placeholder={
                "Des évenements en plus à inclure dans la frise chronologique"
              }
        ...
        {this.state.result !== null ? <TimelineNode data={JSON.parse(this.state.result).data} /> : null}
      </>
    );
  }
}
