...
class Quiz extends React.Component {
  ...
  _sendRequest = () => {
    this.setState({ isLoading: true, error: "", result: null }, async () => {
      const data = await fetch(`${url}/revision-tools/quiz`, {
        method: "POST",
        body: JSON.stringify({
      ...
      if (data.success) {
        ...
        let result = JSON.parse(data.output);
        result.appLocale = {
        ...
        <Container>
          <Paper withBorder radius="md" className={classes.card} bg={"#242537"}>
            ...
            <TextInput
              size={"md"}
              label={"Sujet:"}
              placeholder={"Le sujet sur lequel générer le quiz"}
              mb={"sm"}
            ...
            <Textarea
              ...
              placeholder={"Des éléments / questions en plus à rajouter"}
